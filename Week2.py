...
top_3_words = []
for word in normalized_text:
    if len(top_3_words) < 3:                 #если топ список меньше 3        
        top_3_words.append(word)        #наполнение и сортировка от меньшего к большему по длине
        top_3_words = sorted(top_3_words, key=len)
    elif word not in top_3_words:   #если топ уже заполнен проверяем есть ли уже это слово в словаре
        if len(word) > len(top_3_words[2]):    #Если длина нового слова больше самого длинного слова в топе
            top_3_words[0] = top_3_words[1]
            top_3_words[1] = top_3_words[2]
            top_3_words[2] = word               #сдвигаем два дргих слова на позицию назад, найденное становится самым большим

    Эта другая идея для решения, не доведённая до конца, показалась слишком избыточной
        """
...
def get_len_limit_input():          #проверка на ввод адекватного числа
    while True:
        ...
        try:
            n = int(input())
            if n > 1:           #проверка на ввод отрицательного
                return n
            else:
